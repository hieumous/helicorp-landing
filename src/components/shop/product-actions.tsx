...
export function ProductActions({
  ...
  const onAddCart = () => {
    addToCart(product);
    const ev = buildEvent("shop", "add_to_cart", {
      productId: product.id,
      variant: product.variant,
      price: product.price,
    });
    trackEvent(ev);
    toast.success(`Đã thêm ${product.variant} vào giỏ hàng`);
    openDrawer("cart");
  };
  ...
  const onWishlist = () => {
    ...
    toast(added ? "Đã lưu vào yêu thích" : "Đã bỏ khỏi yêu thích");
  };

  const onView = () => {
    recordView(product);
    trackEvent(buildEvent("shop", "view_product", { productId: product.id }));
    toast.info(`Đang xem: ${product.variant}`);
  };
  ...
  return (
    ...
    >
      <Button size="sm" className="gap-1.5" onClick={onAddCart}>
        ...
        Thêm giỏ — {formatPrice(product.price)}
      </Button>
      <div className="flex gap-2">
        ...
        <Button size="sm" variant="outline" aria-label="Ghi nhận đã xem" onClick={onView}>
  ...
  return (
    <div className="group overflow-hidden rounded-3xl border border-border/60">
      ...
      >
        <Image
          src={product.image}
          alt={`Helix One phiên bản ${product.variant}`}
          fill
